...
namespace VeriTabaniYedeklemeServis.Classes
{
    internal class EMailSenderManager
    {
        internal async static Task MailSendService(string status)
        {
            ...
            {
                TextLog.TextLogging("Mail Alıcı Bilgi Girilmemiştir Mail Gönderilemedi", "");
                return;
            }
            try
            {
    ...
    <body style='font-family: Arial, sans-serif;'>
        <div style='text-align: center; margin-bottom: 20px;'>
            <h1 style='color: #FF0000; margin-bottom: 10px;'>Merhaba!</h1>
            <p style='color: #333; font-size: 16px;'>
        ...
        <footer style='margin-top: 20px; text-align: left;'>
            ...
            {
                TextLog.TextLogging(ex.ToString()+"MAİLDEN GELDİ HATA", "");
                return;
            }
        }
    }
